...
vector<int> mergeArray(vector<int> nums1,int m,vector<int> nums2,int n) {
    ...
    int leftLow=n,leftHigh=m+n-1,rightLow=0,rightHigh=n-1,k=0;
    while(leftLow<=leftHigh && rightLow<=rightHigh) {
        if(nums1[leftLow]<=nums2[rightLow]) nums1[k++]=nums1[leftLow++];
        else nums1[k++]=nums2[rightLow++];
    }
    while(rightLow<=rightHigh) nums1[k++]=nums2[rightLow++];
    return nums1;
}
